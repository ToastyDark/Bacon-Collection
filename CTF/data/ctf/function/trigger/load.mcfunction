# Game
scoreboard objectives add ctf.start trigger
scoreboard objectives add ctf.stop trigger
scoreboard objectives add ctf trigger
scoreboard objectives add ctf.queue trigger
scoreboard objectives add ctf.join trigger
scoreboard objectives add ctf.leave trigger
scoreboard objectives add ctf.invite trigger
scoreboard objectives add ctf.pause trigger

# Map Making
scoreboard objectives add ctf.debug trigger

# Settings
scoreboard objectives add ctf.music trigger
scoreboard objectives add ctf.map trigger
scoreboard objectives add ctf.maps trigger
scoreboard objectives add ctf.map.desert_maze trigger
scoreboard objectives add ctf.map.sandcastle trigger
scoreboard objectives add ctf.settings.random_teams trigger