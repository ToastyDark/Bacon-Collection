...
execute if data storage ctf:game bomb{stolen:true} if data storage ctf:game bomb{team_has:1} unless entity @a[tag=ctf_has_bomb_team1] run function ctf:game/_bomb/diffuse/start with storage ctf:game team1
execute if data storage ctf:game bomb{stolen:true} if data storage ctf:game bomb{team_has:2} unless entity @a[tag=ctf_has_bomb_team2] run function ctf:game/_bomb/diffuse/start with storage ctf:game team2

# Check Bombsite tick
execute as @e[type=armadillo,tag=ctf_bombsite] run function ctf:game/_bomb/site/tick
