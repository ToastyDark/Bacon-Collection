...
$execute as @s[tag=hitBlock] at @s if block ~ ~ ~ #ctf_flag_claw run function ctf:game/flag/claw/block/check_if_flag with storage ctf:game team$(ops_team_num)
# Check if still hitting a Claw Block
$execute as @s[tag=hitBlock,tag=still] at @s if block ~ ~ ~ #ctf_flag_claw run function ctf:game/flag/claw/block/check_if_still with storage ctf:game team$(ops_team_num)
